...
// package/xcolor/index.ts
var xcolor_exports = {};
__export(xcolor_exports, {
  DVI_PS_NAMES: () => DVI_PS_NAMES,
  PREDEFINED_XCOLOR_COLORS: () => PREDEFINED_XCOLOR_COLORS,
  SVG_NAMES: () => SVG_NAMES,
  X11_NAMES: () => X11_NAMES,
  XColorCoreModelToColor: () => XColorCoreModelToColor,
  colorToTextcolorMacro: () => colorToTextcolorMacro,
  computeColor: () => computeColor,
  environments: () => environments,
  macros: () => macros,
  parse: () => parse,
  printRaw: () => printRaw2,
  xcolorColorToHex: () => xcolorColorToHex,
  xcolorMacroToHex: () => xcolorMacroToHex
});
module.exports = __toCommonJS(xcolor_exports);
...
// package/xcolor/libs/xcolor.ts
var CORE_MODELS = /* @__PURE__ */ new Set(["rgb", "cmy", "cmyk", "hsb", "gray"]);
var XColorCoreModelToColor = {
  ...
  hsb: ([h, s2, b]) => (0, import_color2.default)([h * 360, s2 * 100, b * 100], "hsv"),
...
var ColorToXColorModel = {
  rgb: (color) => color.rgb().array().map((v) => v / 255),
  cmy: (color) => [255 - color.red(), 255 - color.green(), 255 - color.blue()].map(
    (v) => v / 255
  ),
  cmyk: (color) => color.cmyk().array().map((v, i) => i === 3 ? v / 100 : v / 255),
  hsb: (color) => [
    color.hue() / 360,
...
function computeColor(expr, predefinedColors = {}) {
  ...
  let computedColor = (0, import_color2.default)("#000000");
  if (color.type === "expr") {
    ...
    for (const mix of color.mix_expr) {
      if (mix.type === "complete_mix") {
        ...
        base = base.mix((0, import_color2.default)("#FFFFFF"), 1 - mix.mix_percent / 100);
      }
    }
  ...
  if (color.type === "extended_expr") {
    ...
    const div = color.div || color.expressions.reduce((a, expr2) => a + expr2.weight, 0);
    if (div <= 0) {
      throw new Error(
        `Cannot mix color with ratios that have a denominator of ${div}`
      );
    }
    const colorsToMix = color.expressions.map(
      (expr2) => [
        expr2.weight / div,
        computeColor({
          type: "color",
...
// package/xcolor/libs/print-raw.ts
function printRaw2(node, root = false) {
  if (typeof node === "string") {
    return node;
  }
  if (Array.isArray(node)) {
    const sepToken = root ? " " : "";
    return node.map((tok) => printRaw2(tok)).join(sepToken);
  }
  ...
  switch (node.type) {
    ...
    case "expr":
      return `${node.prefix || ""}${node.name}${node.mix_expr.map((mix) => printRaw2(mix)).join("")}${node.postfix ? printRaw2(node.postfix) : ""}`;
    case "weighted_expr":
      return `${printRaw2(node.color)},${node.weight}`;
    case "extended_expr":
      let prefix = node.core_model;
      if (node.div) {
        prefix += `,${node.div}`;
      }
      return `${prefix}:${node.expressions.map((expr) => printRaw2(expr)).join(";")}`;
    case "function":
      return `>${node.name},${node.args.map((a) => "" + a).join(",")}`;
    case "color":
      return printRaw2(node.color) + node.functions.map((f) => printRaw2(f)).join("");
    default:
      console.warn(
...
// ../unified-latex-util-comments/dist/index.js
