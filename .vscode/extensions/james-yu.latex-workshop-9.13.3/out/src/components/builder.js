...
class Builder {
    ...
    buildOnFileChanged(file, bibChanged = false) {
        ...
        lw.eventBus.fire(eventbus_1.AutoBuildInitiated, { type: 'onChange', file });
    ...
    buildOnSaveIfEnabled(file) {
        ...
        lw.eventBus.fire(eventbus_1.AutoBuildInitiated, { type: 'onSave', file });
    ...
    async monitorProcess(step, env) {
        ...
        const result = await new Promise(resolve => {
            ...
            this.process.on('exit', async (code, signal) => {
                const isSkipped = parser_1.parser.parseLog(stdout, step.rootFile);
                if (!step.isExternal) {
                ...
                if (!step.isExternal && signal !== 'SIGTERM' && !step.isRetry && configuration.get('latex.autoBuild.cleanAndRetry.enabled')) {
                    ...
                    lw.eventBus.fire(eventbus_1.AutoCleaned);
                }
    ...
    populateTools(rootFile, buildTools) {
        ...
        buildTools.forEach(tool => {
            ...
            Object.entries(env).forEach(([key, value]) => {
                env[key] = value && (0, utils_1.replaceArgumentPlaceholders)(rootFile, lw.manager.tmpDir)(value);
            });
