...
exports.Command = exports.isTriggerSuggestNeeded = void 0;
...
class Command {
    ...
    initialize(environment) {
        ...
        const cmds = excludeDefault ? {} : JSON.parse(fs.readFileSync(`${lw.extensionRoot}/data/commands.json`, { encoding: 'utf8' }));
        const maths = excludeDefault ? {} : JSON.parse(fs.readFileSync(`${lw.extensionRoot}/data/packages/tex.json`, { encoding: 'utf8' })).cmds;
    ...
    parseAst(node, filePath, defined) {
    ...
    parseContent(content, filePath) {
        ...
        while (true) {
            ...
            const cmd = new completerutils_1.CmdEnvSuggestion(`\\${result[1]}${args}`, cmdInPkg.find(candidate => candidate.signatureAsString() === result[1] + args)?.package ?? '', [], -1, { name: result[1], args }, vscode.CompletionItemKind.Function);
