...
exports.outline = exports.construct = void 0;
...
async function parseNode(node, rnwSub, root, filePath, config, structs) {
    const attributes = {
    ...
    if (node.type === 'macro' && config.macros.secs.includes(node.content) && node.args?.[2].openMark === '{') {
        ...
        element = {
            type: node.args?.[0]?.content[0] ? structure_1.TeXElementType.SectionAst : structure_1.TeXElementType.Section,
            name: node.content,
        ...
        const argStr = (0, parser_1.argContentToStr)(node.args?.[1]?.content || []);
        element = {
            type: structure_1.TeXElementType.Command,
            name: node.content,
        ...
        element = {
            type: structure_1.TeXElementType.Environment,
            name: node.env,
        ...
        const subFile = (0, utils_1.resolveFile)([path.dirname(filePath), path.dirname(lw.manager.rootFile || ''), ...config.texDirs], arg0);
        if (subFile) {
            element = {
                type: structure_1.TeXElementType.SubFile,
                name: node.content,
        ...
        const subFile = (0, utils_1.resolveFile)([arg0, path.join(path.dirname(lw.manager.rootFile || ''), arg0)], arg1);
        if (subFile) {
            element = {
                type: structure_1.TeXElementType.SubFile,
                name: node.content,
...
function insertSubFile(structs, struct, traversed) {
    ...
    for (const element of struct) {
        if (element.type === structure_1.TeXElementType.SubFile
...
function nestNonSection(struct) {
    ...
    let currentSection;
    for (const element of struct) {
        if (element.type === structure_1.TeXElementType.Section || element.type === structure_1.TeXElementType.SectionAst) {
            elements.push(element);
...
function addFloatNumber(struct, counter = {}) {
    for (const element of struct) {
        if (element.type === structure_1.TeXElementType.Environment && element.name !== 'macro' && element.name !== 'environment') {
...
function addSectionNumber(struct, config, tag, lowest) {
    ...
    for (const element of struct) {
        ...
        if (element.type === structure_1.TeXElementType.Section) {
        ...
        element.label = `${element.type === structure_1.TeXElementType.Section ? sectionNumber : '*'} ${element.label}`;
        if (element.children.length > 0) {
...
function parseRnwChildCommand(content, file, rootFile) {
    const children = [];
