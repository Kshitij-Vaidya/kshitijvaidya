...
async function buildBibTeX(document) {
    ...
    const ast = await parser_1.parser.parseBibTeX(document.getText());
    if (ast === undefined) {
        return [];
    }
    ...
    ast.content.filter(latex_utensils_1.bibtexParser.isEntry)
        .forEach(entry => {
        const bibitem = {
            type: structure_1.TeXElementType.BibItem,
            name: entry.entryType,
            label: `${entry.entryType}: ${entry.internalKey}`,
            lineFr: entry.location.start.line - 1,
        ...
        entry.content.forEach(field => {
            ...
            const fielditem = {
                type: structure_1.TeXElementType.BibField,
                name: field.name,
