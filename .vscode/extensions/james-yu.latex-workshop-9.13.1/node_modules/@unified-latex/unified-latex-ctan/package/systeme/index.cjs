...
// package/systeme/index.ts
var systeme_exports = {};
__export(systeme_exports, {
  attachSystemeSettingsAsRenderInfo: () => attachSystemeSettingsAsRenderInfo,
  environments: () => environments,
  extractVariables: () => extractVariables,
  macros: () => macros,
  parse: () => parse,
  printRaw: () => printRaw2,
  systemeContentsToArray: () => systemeContentsToArray
});
module.exports = __toCommonJS(systeme_exports);
...
function printRaw2(node, root = false) {
  ...
  switch (node.type) {
    case "annotation":
      return `${printRaw(node.marker)}${printRaw(
        node.content
      )}`;
    case "item":
      return `${node.op ? printRaw(node.op) : ""}${printRaw(
        node.content
      )}`;
    case "equation":
      const left = node.left.map((n) => printRaw2(n)).join("");
      const right = printRaw(node.right);
      const equals = node.equals ? printRaw(node.equals) : "";
      return `${left}${equals}${right}`;
    case "line":
      const equation = node.equation ? printRaw2(node.equation) : "";
      const annotation = node.annotation ? printRaw2(node.annotation) : "";
      const sep = node.sep ? printRaw(node.sep) : "";
      const body = `${equation}${annotation}${sep}`;
      if (node.trailingComment) {
        return printRaw([body, node.trailingComment]);
      }
      return body;
    default:
      console.warn(
...
function sortVariables(vars, whitelistedVariables) {
  ...
  varNames.sort();
  const nameToPos = whitelistedVariables ? new Map(whitelistedVariables.map((v, i) => [printRaw(v), i])) : new Map(varNames.map((name, i) => [name, i]));
  return new Map(
