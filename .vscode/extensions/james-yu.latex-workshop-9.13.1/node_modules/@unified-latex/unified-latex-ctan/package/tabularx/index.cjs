...
function parseTabularSpec(ast) {
  ...
  return TabularPegParser.parse(ast, createMatchers2());
}
    ...
    case "alignment":
      ...
      if (node.alignment === "parbox") {
        if (node.baseline === "top") {
          return `p{${printRaw(node.size)}}`;
        }
        if (node.baseline === "default") {
          return `m{${printRaw(node.size)}}`;
        }
        if (node.baseline === "bottom") {
          return `b{${printRaw(node.size)}}`;
        }
        return `w{${printRaw(node.baseline)}}{${printRaw(
          node.size
        )}}`;
      }
      break;
    case "decl_code":
      return printRaw(node.code);
    case "column":
      const end_code = node.before_end_code ? `<{${printRaw2(node.before_end_code)}}` : "";
      const start_code = node.before_start_code ? `>{${printRaw2(node.before_start_code)}}` : "";
      return [
        printRaw2(node.pre_dividers),
        start_code,
        printRaw2(node.alignment),
        end_code,
        printRaw2(node.post_dividers)
      ].join("");
    default:
      console.warn(
