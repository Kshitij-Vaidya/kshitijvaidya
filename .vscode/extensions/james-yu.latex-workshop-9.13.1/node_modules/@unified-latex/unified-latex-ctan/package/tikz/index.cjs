...
function attachMacroArgsInArray(nodes, macros2) {
  ...
  while (currIndex >= 0) {
    ...
    const macroInfo = macros2[macroName];
...
function printRaw2(node, root = false) {
  ...
  switch (type) {
    case "path_spec":
      return printRaw2(node.content, root = true);
    case "coordinate":
      return `${printRaw(node.prefix)}(${printRaw(
        node.content
      )})`;
    case "operation":
    ...
    case "line_to":
      return node.command;
    case "curve_to": {
      const comments = node.comments.map((c) => printRaw({ ...c, leadingWhitespace: false })).join("");
      if (node.controls.length === 1) {
        return `${comments}.. controls ${printRaw2(
          node.controls[0]
        )} ..`;
      } else {
        return `${comments}.. controls ${printRaw2(
          node.controls[0]
        )} and ${printRaw2(node.controls[1])} ..`;
      }
    }
    case "unknown":
      return printRaw(node.content);
    ...
    case "foreach": {
      ...
      let options = "";
      if (node.options) {
        options = ` [${printRaw(node.options)}]`;
      }
      const start = printRaw(node.start);
      const variables = [...node.variables];
      trim(variables);
      let printedVariables = printRaw(variables);
      if (printedVariables.length > 0) {
        printedVariables = " " + printedVariables;
      }
      const command = node.command.type === "foreach" ? printRaw2(node.command) : printRaw(node.command);
      return `${comments}${start}${printedVariables}${options} in ${printRaw(
        node.list
      )} ${command}`;
    }
    case "svg_operation": {
      ...
      return `${comments}svg${options} ${printRaw(node.content)}`;
    }
    case "animation": {
      const comments = node.comments.map((c) => printRaw({ ...c, leadingWhitespace: false })).join("");
      return `${comments}:${node.attribute} = {${printRaw(
        node.content
      )}}`;
    }
    default:
