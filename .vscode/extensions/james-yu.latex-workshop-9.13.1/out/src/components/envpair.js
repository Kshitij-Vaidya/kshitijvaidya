...
class EnvPair {
    constructor() { }
    async buildCommandPairTree(document) {
        const ast = await parser_2.parser.parseLaTeX(document.getText());
        if (!ast) {
    ...
    buildCommandPairTreeFromNode(doc, node, next, parentCommandPair, commandPairs) {
        ...
        if (node.type === 'environment' || node.type === 'mathenv') {
            ...
            const name = (0, parser_1.argContentToStr)([node.env]) || node.env;
